...
// StorageGoogleCloud stores data in a Google Cloud Storage bucket
type StorageGoogleCloud struct {
...
func init() {
	knoxite.RegisterBackendFactory(&StorageGoogleCloud{})
}

// NewBackend returns a StorageGoogleCloud backend
...
func (*StorageGoogleCloud) NewBackend(URL url.URL) (knoxite.Backend, error) {
	...
	if URL.User == nil || URL.User.Username() == "" {
		...
		if !credentialsSet {
			return &StorageGoogleCloud{}, errors.New("No valid JSON credentials file provided.")
		}
	} else {
	...
	if len(slicedPath) > 2 {
		...
		return &StorageGoogleCloud{}, knoxite.ErrInvalidRepositoryURL
	}
	...
	backend := StorageGoogleCloud{
		url:    URL,
		client: *client,
		bucket: *bucket,
	}

	knfs, err := knoxite.NewStorageFilesystem(folderPath, &backend)
	if err != nil {
		return &StorageGoogleCloud{}, err
	}
...
func (backend *StorageGoogleCloud) Location() string {
	return backend.url.String()
}

// Close the backend
func (backend *StorageGoogleCloud) Close() error {
	return backend.client.Close()
}

// Protocols returns the Protocol Schemes supported by this backend
func (backend *StorageGoogleCloud) Protocols() []string {
	return []string{"googlecloudstorage"}
}

// Description returns a user-friendly description for this backend
func (backend *StorageGoogleCloud) Description() string {
	return "Google Cloud Storage"
}

// AvailableSpace returns the free space on this backend
func (backend *StorageGoogleCloud) AvailableSpace() (uint64, error) {
...
func (backend *StorageGoogleCloud) CreatePath(path string) error {
	return nil
}

// Stat returns the size of a file
func (backend *StorageGoogleCloud) Stat(path string) (uint64, error) {
...
func (backend *StorageGoogleCloud) ReadFile(path string) ([]byte, error) {
...
func (backend *StorageGoogleCloud) WriteFile(path string, data []byte) (size uint64, err error) {
...
func (backend *StorageGoogleCloud) DeleteFile(path string) error {
